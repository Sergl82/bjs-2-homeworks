describe('Домашнее задание к занятию 1. «Основные понятия»', () => {
  describe('Задача №1', () => {
    ...
    it('возвращать строку c названием ошибки, если неверно передали первый параметр', () => {
      ...
      const percent = 'test';
      expect(calculateTotalMortgage(percent, 0, 10000, nextYearDate)).toEqual(`Параметр 
      "Процентная ставка" содержит неправильное значение "${percent}"`);
    });

    it('возвращать строку c названием ошибки, если неверно передали второй параметр', () => {
      const nextYearDate = new Date(new Date().setFullYear(new Date().getFullYear() + 3));
      const contribution = 'test';
      expect(calculateTotalMortgage(15, contribution, 10000, nextYearDate)).toEqual(`Параметр
       "Начальный взнос" содержит неправильное значение "${contribution}"`);
    });

    it('возвращать строку c названием ошибки, если неверно передали третий параметр', () => {
      const nextYearDate = new Date(new Date().setFullYear(new Date().getFullYear() + 3));
      const amount = 'test';
      expect(calculateTotalMortgage(15, 0, amount, nextYearDate)).toEqual(`Параметр "Общая стоимость"
